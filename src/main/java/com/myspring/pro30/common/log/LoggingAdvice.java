...
@Component //빈등록
@Aspect		//관점 지향저인 프로그래밍을 위한 패턴
public class LoggingAdvice {
	private static final Logger logger = LoggerFactory.getLogger(LoggingAdvice.class);

	// target 메서드의 파라미터등 정보를 출력합니다.
	// advice : advice의 실제 구현체 
	// @Before : advice의 동작 시점
	//				  리턴 타입  경로        패키지 경로           클래스.모든 메서드  --->범위를 지정  
	@Before("execution(* com.myspring.pro30.*.service.*.*(..)) or "
	        + "execution(* com.myspring.pro30.*.dao.*.*(..))") 
	//point cut : advice 적용되는  지점
	public void startLog(JoinPoint jp) {	
		//joint point : advice가 적용되는 지점의 메서드 
		logger.info("-------------------------------------");
		logger.info("-------------------------------------");
